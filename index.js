...
// Memvalidasi autentikasi dan otorisasi role pengguna dari header request
  ...
  return async (req, res, next) => {
    try {
      ...
      if (!userId || !sessionToken) {
        return res.status(401).json({ message: "Akses ditolak. User belum login." });
      }
      ...
      return res.status(500).json({ message: "Gagal validasi akses.", detail: err.message });
    }
  };
};
...
// Mengambil ID cabang utama beserta sub-cabang yang berada di bawahnya
const getAllowedCabangIdsForManager = async (managerCabangId) => {
  ...
  return [managerCabangId, ...(subBranches || []).map((branch) => branch.id)];
};

// Mendapatkan waktu standar WIB (Waktu Indonesia Barat)
...
// Memformat payload data cabang sebelum disimpan ke database
...
// Membersihkan payload data pengguna dari field yang tidak relevan
const sanitizeUserPayload = (body) => {
  ...
  payload.role = VALID_ROLES.includes(payload.role) ? payload.role : "karyawan";
  payload.status = VALID_USER_STATUSES.includes(payload.status) ? payload.status : "Aktif";
...
// Memvalidasi aturan bisnis untuk penempatan role pengguna
const validateUserRoleRules = async (payload, currentUserId = null) => {
  if (!payload.role) {
    return { valid: false, status: 400, message: "Hak akses wajib dipilih." };
  }

  if (!VALID_ROLES.includes(payload.role)) {
    return { valid: false, status: 400, message: "Hak akses tidak valid." };
  }
  ...
  const isStatusActive = (payload.status || "Aktif") === "Aktif";
  ...
  if (payload.role === "hrd") {
    ...
    const otherActiveHrd = (existingHrd || []).filter((item) => item.id !== currentUserId);

    if (isStatusActive && otherActiveHrd.length > 0) {
      return {
        valid: false,
        status: 400,
  ...
  if (payload.role === "managerCabang") {
    if (!payload.cabang_id) {
      return { valid: false, status: 400, message: "Manager Cabang wajib ditempatkan pada cabang utama." };
    }
    ...
    if (cabangError || !cabang) {
      return { valid: false, status: 400, message: "Cabang untuk Manager Cabang tidak ditemukan." };
    }

    if (cabang.parent_id) {
      return { valid: false, status: 400, message: "Manager Cabang hanya boleh ditempatkan pada cabang utama, bukan sub-cabang." };
    }
    ...
    const otherManager = (existingManager || []).filter((item) => item.id !== currentUserId);

    if (isStatusActive && otherManager.length > 0) {
      return {
        valid: false,
        status: 400,
...
app.get("/api/cabang", requireRole("hrd", "managerCabang"), async (req, res) => {
  try {
    let query = supabase.from("cabang").select("*").order("nama", { ascending: true });

    if (req.user.role === "managerCabang") {
      const allowedCabangIds = await getAllowedCabangIdsForManager(req.user.cabang_id);
    ...
    if (error) {
      return res.status(500).json({ message: "Gagal mengambil cabang", detail: error.message });
    }

    res.status(200).json(data || []);
  } catch (err) {
    res.status(500).json({ message: "Gagal mengambil cabang", detail: err.message });
  }
});
...
app.post("/api/cabang", requireRole("hrd"), async (req, res) => {
  try {
    ...
    if (error) {
      return res.status(400).json({ message: "Gagal menambah cabang", detail: error.message });
    }
...
app.get("/api/karyawan", requireRole("hrd", "managerCabang"), async (req, res) => {
  try {
    let selectFields = "id, nik, nama, role, jabatan, divisi, tempat_lahir, tanggal_lahir, jenis_kelamin, tanggal_masuk, status, alamat, no_telp, cabang_id, foto_karyawan, ktp, kk, skck, sim, sertifikat, dokumen_tambahan, cabang(id, nama, parent_id)";

    // Hanya HRD yang dapat melihat field password
    if (req.user.role === "hrd") {
      selectFields = selectFields.replace("nik, nama", "nik, password, nama");
    }

    let query = supabase.from("users").select(selectFields).order("nama", { ascending: true });

    if (req.user.role === "managerCabang") {
      const allowedCabangIds = await getAllowedCabangIdsForManager(req.user.cabang_id);
      query = query.in("cabang_id", allowedCabangIds).neq("role", "hrd");
    }

    const { data, error } = await query;

    if (error) {
      return res.status(500).json({ message: "Gagal mengambil data karyawan", detail: error.message });
    }
    ...
    res.status(500).json({ message: "Gagal mengambil data karyawan", detail: err.message });
  }
});
...
app.post("/api/karyawan", requireRole("hrd"), async (req, res) => {
  try {
    ...
    if (error) {
      return res.status(400).json({ message: "Gagal menambah data", detail: error.message });
    }
...
app.put("/api/karyawan/:id", requireRole("hrd"), async (req, res) => {
  try {
    ...
    if (error) {
      return res.status(400).json({ message: "Gagal mengubah data", detail: error.message });
    }
...
app.put("/api/karyawan/:id/status", requireRole("hrd"), async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Gagal mengubah status karyawan", detail: err.message });
  }
});

// ============================================================================
// ROUTES: ABSENSI & RIWAYAT
// ============================================================================

app.post("/api/absensi", requireRole("karyawan", "managerCabang", "hrd"), async (req, res) => {
  const { user_id, tipe_absen, waktu, foto, waktu_istirahat_mulai, waktu_istirahat_selesai } = req.body;

  const targetUserId = (req.user.role === "hrd" && user_id) ? user_id : req.user.id;
  ...
  try {
    ...
    if (userError || !user) {
      return res.status(404).json({ message: "Data user tidak ditemukan.", detail: userError?.message });
    }
    ...
    // Pemrosesan tipe absen menggunakan Switch-Case untuk clean code
    switch (tipe_absen) {
      case "Masuk":
      ...
      case "Istirahat":
        ...
        if (existingAttendance.waktu_istirahat_mulai) {
          return res.status(400).json({ message: "Jadwal istirahat sudah diatur sebelumnya." });
        }
...
app.post("/api/absensi/manual", requireRole("hrd"), async (req, res) => {
  ...
  try {
    ...
    if (userError || !user) {
      return res.status(404).json({ message: "Data karyawan tidak ditemukan.", detail: userError?.message });
    }
    ...
    res.status(500).json({ message: "Gagal menyimpan absensi manual", detail: err.message });
  }
});

app.get("/api/riwayat/:user_id", requireRole("karyawan", "managerCabang", "hrd"), async (req, res) => {
  const userId = req.params.user_id;

  try {
    if (req.user.role === "karyawan" && userId !== req.user.id) {
      return res.status(403).json({ message: "Tidak boleh melihat riwayat orang lain." });
    }

    if (req.user.role === "managerCabang") {
      const allowedCabangIds = await getAllowedCabangIdsForManager(req.user.cabang_id);
      const { data: targetUser } = await supabase.from("users").select("cabang_id, role").eq("id", userId).single();

      if (!targetUser || targetUser.role === "hrd") {
        return res.status(403).json({ message: "Manager tidak boleh melihat riwayat HRD." });
      }
      if (!allowedCabangIds.includes(targetUser.cabang_id)) {
        return res.status(403).json({ message: "Manager tidak boleh melihat riwayat cabang lain." });
      }
    }

    const { data: user } = await supabase.from("users").select("id, cabang_id, cabang(nama)").eq("id", userId).single();
    ...
    const isPusat = user?.cabang?.nama?.toLowerCase().includes("amaga") ||
                    user?.cabang?.nama?.toLowerCase().includes("pusat") ||
                    !user?.cabang_id;

    // Generate absen alpha buatan untuk 30 hari ke belakang jika tidak ada absensi/perizinan
    for (let i = 0; i <= 30; i++) {
      ...
      const hasPermission = (permissionList || []).some(
        (perm) => perm.status_approval === "Disetujui" && dateKey >= perm.tanggal_mulai && dateKey <= perm.tanggal_selesai
      );
    ...
    res.status(500).json({ message: "Gagal mengambil riwayat", detail: err.message });
  }
});
...
app.post("/api/perizinan", requireRole("karyawan", "managerCabang", "hrd"), async (req, res) => {
  try {
    const body = { ...req.body };
    const payload = {
      user_id: req.user.role === "hrd" && body.user_id ? body.user_id : req.user.id,
    ...
    if (!["Izin", "Cuti", "FIMTK"].includes(payload.kategori)) {
      return res.status(400).json({ message: "Kategori perizinan tidak valid.", detail: `Kategori diterima: ${payload.kategori}` });
    }
    ...
    if (error) {
      return res.status(400).json({ message: "Gagal mengirim pengajuan.", detail: error.message });
    }

    return res.status(201).json({ message: "Pengajuan berhasil dikirim." });
  } catch (err) {
    return res.status(500).json({ message: "Gagal mengirim pengajuan.", detail: err.message });
  }
});

app.get("/api/perizinan/all", requireRole("hrd", "managerCabang"), async (req, res) => {
  try {
    let query = supabase
      .from("perizinan")
      .select("*, users!inner(id, nama, nik, role, jabatan, divisi, no_telp, cabang_id, cabang(nama))")
      .order("created_at", { ascending: false });

    if (req.user.role === "managerCabang") {
      const allowedCabangIds = await getAllowedCabangIdsForManager(req.user.cabang_id);
      query = query.in("users.cabang_id", allowedCabangIds).neq("users.role", "hrd");
    }

    const { data, error } = await query;

    if (error) {
      return res.status(500).json({ message: "Gagal mengambil perizinan", detail: error.message });
    }

    res.status(200).json(data || []);
  } catch (err) {
    res.status(500).json({ message: "Gagal mengambil perizinan", detail: err.message });
  }
});

app.put("/api/perizinan/:id/status", requireRole("hrd", "managerCabang"), async (req, res) => {
  try {
    const { status_approval } = req.body;

    if (!["Pending", "Disetujui", "Ditolak"].includes(status_approval)) {
      return res.status(400).json({ message: "Status approval tidak valid.", detail: `Status diterima: ${status_approval}` });
    }

    const { data: izin, error: izinError } = await supabase.from("perizinan").select("id, user_id, status_approval").eq("id", req.params.id).single();

    if (izinError || !izin) {
      return res.status(404).json({ message: "Data perizinan tidak ditemukan.", detail: izinError?.message });
    }

    const { data: pemohon, error: pemohonError } = await supabase.from("users").select("id, cabang_id, role").eq("id", izin.user_id).single();

    if (pemohonError || !pemohon) {
      return res.status(404).json({ message: "Data pemohon perizinan tidak ditemukan.", detail: pemohonError?.message });
    }

    if (req.user.role === "managerCabang") {
      if (pemohon.role === "hrd") {
        return res.status(403).json({ message: "Manager tidak boleh memproses perizinan HRD." });
      }
      const allowedCabangIds = await getAllowedCabangIdsForManager(req.user.cabang_id);
      if (!allowedCabangIds.includes(pemohon.cabang_id)) {
        return res.status(403).json({ message: "Manager tidak boleh memproses perizinan cabang lain." });
      }
    }

    const { error: updateError } = await supabase.from("perizinan").update({ status_approval }).eq("id", req.params.id);

    if (updateError) {
      return res.status(400).json({ message: "Gagal mengubah status perizinan.", detail: updateError.message });
    }

    return res.status(200).json({ message: `Perizinan berhasil di-${status_approval}.` });
  } catch (err) {
    return res.status(500).json({ message: "Gagal mengubah status perizinan.", detail: err.message });
  }
});

app.get("/api/manager/perizinan/:cabang_id", requireRole("managerCabang", "hrd"), async (req, res) => {
  ...
  try {
    if (req.user.role === "managerCabang") {
      ...
      if (!allowedCabangIds.includes(cabangParam)) {
        return res.status(403).json({ message: "Manager tidak boleh akses perizinan cabang lain." });
      }
    }

    let query = supabase
      .from("perizinan")
      .select("*, users!inner(id, nama, nik, role, jabatan, divisi, cabang_id, cabang(nama))")
      .eq("users.cabang_id", cabangParam)
      .order("created_at", { ascending: false });

    if (req.user.role === "managerCabang") query = query.neq("users.role", "hrd");

    const { data, error } = await query;
    if (error) return res.status(500).json({ message: "Gagal mengambil perizinan", detail: error.message });

    return res.status(200).json(data || []);
  } catch (err) {
    return res.status(500).json({ message: "Gagal mengambil perizinan", detail: err.message });
  }
});

// ============================================================================
// ROUTES: LAPORAN & DASHBOARD
// ============================================================================

app.get("/api/laporan", requireRole("hrd", "managerCabang"), async (req, res) => {
  ...
  // Set default periode laporan (tanggal 26 bulan lalu s/d tanggal 25 bulan ini)
  if (!startStr || !endStr) {
  ...
  try {
    ...
    if (req.user.role === "managerCabang") {
      const allowedCabangIds = await getAllowedCabangIdsForManager(req.user.cabang_id);
      userQuery = userQuery.in("cabang_id", allowedCabangIds).neq("role", "hrd");
    }
    ...
    // Menyusun laporan akumulasi untuk masing-masing user
    ...
    res.status(500).json({ message: "Gagal mengambil laporan", detail: err.message });
  }
});

app.get("/api/dashboard/stats", requireRole("hrd", "managerCabang"), async (req, res) => {
  const { sub_cabang } = req.query;

  try {
    let userQuery = supabase.from("users").select("id, role, cabang_id, cabang(nama)");

    if (req.user.role === "managerCabang") {
      const allowedCabangIds = await getAllowedCabangIdsForManager(req.user.cabang_id);
      userQuery = userQuery.in("cabang_id", allowedCabangIds).neq("role", "hrd");
    } else if (req.user.role === "hrd" && sub_cabang && sub_cabang !== "Semua Cabang") {
    ...
    const allBranchLabels = ["Semua Cabang", "Semua Sub-Cabang", "Semua Cabang Saya"];
    const users = sub_cabang && !allBranchLabels.includes(sub_cabang)
    ...
    if (userIds.length === 0) return res.status(200).json(emptyResponse);
    ...
    const chart = {
      hadir: Array(7).fill(0), sakit: Array(7).fill(0), izin: Array(7).fill(0),
      cuti: Array(7).fill(0), terlambat: Array(7).fill(0), alpha: Array(7).fill(0),
    };

    let alphaCounter = 0;

    // Kalkulasi Alpha untuk Chart 7 Hari
    for (let i = 0; i <= 6; i++) {
    ...
    // Kalkulasi Kehadiran & Keterlambatan
    ...
    res.status(500).json({ message: "Gagal mengambil statistik", detail: err.message });
  }
});
...
// Job otomatis berjalan setiap hari pukul 00:00 untuk membersihkan storage foto
