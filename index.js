...
app.get("/api/karyawan", requireRole("hrd", "managerCabang"), async (req, res) => {
  ...
  if (req.user.role === "managerCabang") {
    query = query.eq("cabang_id", req.user.cabang_id);
  }
...
app.get("/api/riwayat/:user_id", requireRole("karyawan", "managerCabang", "hrd"), async (req, res) => {
  ...
  try {
    ...
    if (req.user.role === "managerCabang") {
      ...
      if (!targetUser || targetUser.cabang_id !== req.user.cabang_id) {
        return res.status(403).json({
...
app.get("/api/perizinan/all", requireRole("hrd"), async (req, res) => {
  const { data, error } = await supabase
    .from("perizinan")
    .select("*, users(id, nama, nik, role, jabatan, divisi, cabang_id, cabang(nama))")
    .order("created_at", { ascending: false });
...
app.put("/api/perizinan/:id/status", requireRole("hrd", "managerCabang"), async (req, res) => {
  try {
    ...
    if (req.user.role === "managerCabang" && izin.users?.cabang_id !== req.user.cabang_id) {
      return res.status(403).json({
...
app.get("/api/manager/perizinan/:cabang_id", requireRole("managerCabang", "hrd"), async (req, res) => {
  ...
  if (req.user.role === "managerCabang" && Number(req.user.cabang_id) !== cabangParam) {
    return res.status(403).json({
...
app.get("/api/laporan", requireRole("hrd", "managerCabang"), async (req, res) => {
  ...
  try {
    let userQuery = supabase
      .from("users")
      .select("id, nama, nik, cabang_id, cabang(*)");

    if (req.user.role === "managerCabang") {
      userQuery = userQuery.eq("cabang_id", req.user.cabang_id);
    }
    ...
    const report = (users || []).map((user) => {
      ...
      const userAttendance = (attendanceData || []).filter(
        (a) => a.user_id === user.id && a.tanggal >= startStr && a.tanggal <= endStr
      );

      const userPermissions = (permissionData || []).filter(
        (p) => p.user_id === user.id && p.tanggal_selesai >= startStr && p.tanggal_mulai <= endStr
      );
...
app.get("/api/dashboard/stats", requireRole("hrd", "managerCabang"), async (req, res) => {
  ...
  try {
    let userQuery = supabase.from("users").select("id, cabang_id, cabang(nama)");
    ...
    for (let i = 0; i <= 6; i++) {
      ...
      users.forEach((user) => {
        ...
        const hasPermission = (approvedPermissions || []).some(
          (p) => p.user_id === user.id && dateKey >= p.tanggal_mulai && dateKey <= p.tanggal_selesai
        );
